...
def run_integrated_analysis():
    ...
    for i, TICKER in enumerate(TICKERS):
        ...
        # --- PREVIOUS DAY LEVELS (POD 3 ANCHORS) ---
        ...
        with cols[i]:
            ...
            fig = go.Figure()
            # Dynamic Price Line
            fig.add_trace(go.Scatter(x=df_today.index, y=df_today['Close'], name="Price", line=dict(color=line_color, width=2.5)))
            # VWAP
            fig.add_trace(go.Scatter(x=df_today.index, y=df_today['vwap'], name="VWAP", line=dict(color='blue', width=1.5, dash='dash')))
            # Sigma Bands
            fig.add_hline(y=ub, line_color="green", line_dash="dash", annotation_text="UB")
            fig.add_hline(y=lb, line_color="red", line_dash="dash", annotation_text="LB")
            # PH/PL Lines
...
# --- UI ---
if st.sidebar.button("🚀 Execute Alpha Scan"):
...
if st.session_state.active_trades:
    ...
    # Summary Metrics
    c1, c2 = st.columns(2)
    c1.metric("Total Points", f"{df_res['Live PnL'].sum():,.2f}")
    c2.metric("Active Triggers", len(df_res))
