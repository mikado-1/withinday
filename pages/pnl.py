...
def add_trade(ticker, pod, side, row, ub, lb, marker, df_today, ltp, sl_ovr=None):
    ...
    st.session_state.active_trades.append({
        ...
        "Status": status, "PnL_Pts": round((ltp - entry) * mult, 2)
    })

# --- 3. UI ---
if st.sidebar.button("🔍 Execute Session Scan"):
...
if st.session_state.active_trades:
    ...
    # Summary Table
    st.subheader("🏢 Pod Performance Tracker")
    ...
    # P&L Curve
    st.subheader("📊 Session Equity Curve (Points)")
    df_res['Cum_Points'] = df_res['PnL_Pts'].cumsum()
    st.plotly_chart(go.Figure(go.Scatter(y=df_res['Cum_Points'], fill='tozeroy', line=dict(color='#00CC96', width=3))).update_layout(height=250), use_container_width=True)
