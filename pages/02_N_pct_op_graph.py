...
# --- CONFIG ---
st.set_page_config(page_title="NSE Last Session Pro", layout="wide")
...
def fetch_analysis(symbol, session):
    ...
    try:
        ...
        # Instead of filtering for "today", we take the last date present in the data
...
st.title("📈 NSE Last Available Session Momentum")

if nifty_open:
    atm_anchor = round(nifty_open / 50) * 50
    ...
    with st.spinner("Retrieving Last Session Data..."):
        for s in strikes:
            ce = fetch_analysis(f"OPTIDXNIFTY{expiry}CE{float(s):.2f}", session)
            pe = fetch_analysis(f"OPTIDXNIFTY{expiry}PE{float(s):.2f}", session)
    ...
    if ce_trend_list and pe_trend_list:
        ...
        # 📊 Scanner Table
        st.divider()
        st.subheader("📋 R² & Day Slope Scanner")
        ...
        # 📍 Individual Strike Profiles
        st.divider()
        st.subheader("📍 Individual Strike Trending")
    ...
    st.error("Could not retrieve Nifty Open. API may be offline.")
